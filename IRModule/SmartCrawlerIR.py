...
import ConfigParser
import os
import ParserTRECFormat
from whoosh.lang.porter import stem


#Retrieves the top K most similar tweets given a query
#@input query - the raw query from the interface
#@input replies - a txt file containing all the replies from the given tweet. Every tweet should be in one different line in the input file
#@output a list of similar tweets from our dataset
def retrieveTopKSimilarTweets(query, replies):
...
def indexDocuments(replies):
    print "Indexing....................................................................................."

    #Transform the documents into TREC Format
    ParserTRECFormat.parser(replies)

    indexDataset = os.path.abspath("repliesTRECFormat.txt")
    ...
    if not os.listdir(terrierLocation + "\\var\\index\\") ==[]:
        for file in os.listdir(terrierLocation + "\\var\\index\\"):
            toErase =terrierLocation + "\\var\\index\\" + file
            os.remove(toErase.replace("\\", "\\\\"))

    #Index the replies file
    subprocess.check_output('cd ' + terrierLocation + '\\bin' +
...
if __name__ == "__main__":
    replies = "rawTweets.txt"
    retrieveTopKSimilarTweets('Zika symptoms are irelevant', replies);
